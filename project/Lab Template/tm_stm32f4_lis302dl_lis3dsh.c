...
void TM_LIS302DL_LIS3DSH_INT_ShortDelay(void) {
	uint32_t delay = 100;
	while (delay--);
}
...
void TM_LIS302DL_LIS3DSH_INT_ReadSPI(uint8_t* data, uint8_t addr, uint8_t count) {
	...
	/* Enable Multi-Read */
	//uint8_t cr6_addr[1UL] = { 0x10 };
	//if(count > 1)
	//	TM_LIS302DL_LIS3DSH_INT_WriteSPI(cr6_addr,LIS3DSH_CTRL_REG6_ADDR,1);
...
void TM_LIS3DSH_INT_ReadAxes(TM_LIS302DL_LIS3DSH_t *Axes_Data) 
{
	/* Local variables */
	uint8_t dbuffer[6UL];
	//dbuffer[0] <- REG LIS3DSH_OUT_X_L_ADDR = 0x028 | OUT_X_L
	//dbuffer[1] <- REG LIS3DSH_OUT_X_H_ADDR = 0x029 | OUT_X_H
	//dbuffer[2] <- REG LIS3DSH_OUT_Y_L_ADDR = 0x02A | OUT_Y_L
	//dbuffer[3] <- REG LIS3DSH_OUT_Y_H_ADDR = 0x02B | OUT_Y_H
	//dbuffer[4] <- REG LIS3DSH_OUT_Z_L_ADDR = 0x02A | OUT_Z_L
	//dbuffer[5] <- REG LIS3DSH_OUT_Z_H_ADDR = 0x02B | OUT_Z_H
	
	/* Read data from sensor over SPI using 
	the TM_LIS302DL_LIS3DSH_INT_ReadSPI function */
	TM_LIS302DL_LIS3DSH_INT_ReadSPI((uint8_t*)(dbuffer+0UL),LIS3DSH_OUT_X_L_ADDR,6UL);
	/*TM_LIS302DL_LIS3DSH_INT_ReadSPI((uint8_t*)(dbuffer+1UL),LIS3DSH_OUT_X_H_ADDR,1UL);
	TM_LIS302DL_LIS3DSH_INT_ReadSPI((uint8_t*)(dbuffer+2UL),LIS3DSH_OUT_Y_L_ADDR,1UL);
	TM_LIS302DL_LIS3DSH_INT_ReadSPI((uint8_t*)(dbuffer+3UL),LIS3DSH_OUT_Y_H_ADDR,1UL);
	TM_LIS302DL_LIS3DSH_INT_ReadSPI((uint8_t*)(dbuffer+4UL),LIS3DSH_OUT_Z_L_ADDR,1UL);
	TM_LIS302DL_LIS3DSH_INT_ReadSPI((uint8_t*)(dbuffer+5UL),LIS3DSH_OUT_Z_H_ADDR,1UL);
	*/
	/* Put received data into Axes_Data */
	#define CONV16(bL,bH)  (short int)( ( (short int)bL ) | ( ( (short int)bH ) << 8UL ) )   
	
	Axes_Data->X = CONV16(dbuffer[0U],dbuffer[1U])*TM_LIS3DSH_INT_Sensitivity;
	Axes_Data->Y = CONV16(dbuffer[2U],dbuffer[3U])*TM_LIS3DSH_INT_Sensitivity;
	Axes_Data->Z = CONV16(dbuffer[4U],dbuffer[5U])*TM_LIS3DSH_INT_Sensitivity;
